...
APTR DevClose(struct IOSana2Req *req reg(a1), struct DevData *dd reg(a6))
{
    ...
    if (req->ios2_BufferManagement) FreeMem(req->ios2_BufferManagement, sizeof(struct BuffFunctions));
...
LONG RunTask(struct DevData *dd, struct UnitData *ud)
{
    ...
    if (ud->ud_LifeTime = CreateMsgPort())
    {
        if (ud->ud_Task = (struct Task *)CreateNewProcTags(
                NP_Entry, (LONG)UnitTask, NP_Name, (LONG)ud->ud_Name, NP_Priority, 6, TAG_END))
        {
            WORD i;
...
void BufferOverflow(struct UnitData *ud)
{
    ...
    if (resent) hw->regs[NE2000_COMMAND] = COMMAND_PAGE0 | COMMAND_START | COMMAND_ABORT | COMMAND_TXP;
