...
static BOOL init_resources(struct CardBase *cb)
{
    ...
    if (!(cb->cb_PrometheusBase = OpenLibrary("prometheus.library", 2))) return FALSE;
    if (!(cb->cb_ExpansionBase = OpenLibrary("expansion.library", 39))) return FALSE;
    ...
    if (!(cb->cb_MemPool = CreatePool(MEMF_CLEAR | MEMF_PUBLIC, 1024, 512))) return FALSE;

    /* setting up memory list semaphore */

    if (!(cb->cb_MemSem = AllocPooled(cb->cb_MemPool, sizeof(struct SignalSemaphore)))) return FALSE;
