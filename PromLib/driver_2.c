...
struct PrometheusBase *LibInit(__reg("d0") struct PrometheusBase *pb,
                               __reg("a0") void *                 seglist,
                               __reg("a6") struct Library *       sysb);
...
PCIBoard *             FindBoardTagList(__reg("a6") struct PrometheusBase *pb,
                                        __reg("a0") PCIBoard *             node,
                                        __reg("a1") struct TagItem *       taglist);
ULONG                  GetBoardAttrsTagList(__reg("a6") struct PrometheusBase *pb,
                                            __reg("a0") PCIBoard *             board,
                                            __reg("a1") struct TagItem *       taglist);
...
void  WriteConfigLong(__reg("a6") struct PrometheusBase *pb,
                      __reg("a0") PCIBoard *             board,
                      __reg("d0") ULONG                  data,
                      __reg("d1") UBYTE                  offset);
void  WriteConfigWord(__reg("a6") struct PrometheusBase *pb,
                      __reg("a0") PCIBoard *             board,
                      __reg("d0") UWORD                  data,
                      __reg("d1") UBYTE                  offset);
void  WriteConfigByte(__reg("a6") struct PrometheusBase *pb,
                      __reg("a0") PCIBoard *             board,
                      __reg("d0") UBYTE                  data,
                      __reg("d1") UBYTE                  offset);
LONG  SetBoardAttrsTagList(__reg("a6") struct PrometheusBase *pb,
                           __reg("a0") PCIBoard *             board,
                           __reg("a1") struct TagItem *       taglist);
BOOL  AddIntServer_(__reg("a6") struct PrometheusBase *pb,
                    __reg("a0") PCIBoard *             board,
                    __reg("a1") struct Interrupt *     intr);
void  RemIntServer_(__reg("a6") struct PrometheusBase *pb,
                    __reg("a0") PCIBoard *             board,
                    __reg("a1") struct Interrupt *     intr);
...
APTR  AllocPCIAddressSpace(__reg("a6") struct PrometheusBase *pb,
                           __reg("a0") PCIBoard *             board,
                           __reg("d0") ULONG                  size,
                           __reg("d1") ULONG                  bar);
...
struct InitTable /* do not change */
{
    ...
    APTR               InitLibTable;
} InitTab = {(ULONG)sizeof(struct PrometheusBase), (APTR *)&FuncTable[0], (struct MyDataInit *)&DataTab, (APTR)LibInit};
...
void AddRequest(struct PrometheusBase *pb,
                struct PCIBus *        pcibus,
                ULONG                  size,
                ULONG                  type,
                struct TagItem *       tag,
                ULONG volatile *       cfgreg,
                ULONG                  flag)
{
    struct Library * SysBase = pb->pb_SysBase;
    struct SpaceReq *srq;
...
void QueryCard(struct PrometheusBase *    pb,
               struct PCIBus *            pcibus,
               volatile struct PciConfig *conf,
        ...
        if (pcinode = AllocPooled(pb->pb_MemPool, sizeof(PCIBoard)))
        {
              ...
              flag = 1;
                    ...
                    if (memsize)
                    {
                        ...
                        AddRequest(pb,
                                   ...
                                   memsize,
                                   memtype,
                                   &pcinode->pn_TagList[tagindex++],
                                   &conf->types.t0.pc_BaseRegs[basereg],
                                   flag);
                    }
                    else
                    {
            ...
            if (memsize)
            {
                ...
                AddRequest(pb,
                           ...
                           memtype,
                           &pcinode->pn_TagList[tagindex++],
                           Bridge ? &conf->types.t1.pc_ROM : &conf->types.t0.pc_ROM,
                           0);
            }
        }
...
void ConfigurePrometheus(struct PrometheusBase *pb, struct ConfigDev *cdev)
{
    ...
    for (slot = 0; slot < 4; slot++)
    {
        ...
        if (((headertype & 0x7f) == 0x01) && (pb->pb_FireStorm == TRUE))
        {
            ...
            if ((pci->types.t1.pc_IOBase & 0xf) == 0x01) pbusnew->br_hasUpperIO = TRUE;
...
APTR GetVirtualAddress(__reg("a6") struct PrometheusBase *pb, __reg("d0") APTR addr)
{
    ...
    if (pb->pb_FireStorm == TRUE)
    {
        if ((ULONG)addr >= FS_PCI_ADDR_CONFIG0)
            ...
            return (APTR)((ULONG)addr + (ULONG)pb->pb_BaseAddr);
    }
    else
    {
